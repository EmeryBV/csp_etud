...
//		System.out.println(varDom);
	...
	public void triVariable() {
		LinkedHashMap<String,List<Object>> newVarDom = new LinkedHashMap<>(); // HashMap trier
		HashMap<String, Integer> compteur = new HashMap<String, Integer>(); // HashMap qui permet de compter le nombre de contrainte par variable
		for (String key : varDom.keySet()) {
			compteur.put(key, 0); // initialsaiton des keys avec compteur à 0
		}

		for (Constraint contraint : constraints) {
			for (String key : varDom.keySet()) {
				if (contraint.varList.contains(key)) {
					compteur.put(key, compteur.get(key) + 1); // pour chaque contrainte qui contient le domaine, compteur ++
				}
			}
...
//        System.out.println(compteur);
		...
		for (String keyC : compteur.keySet()) { // reaffecte les domaines dans la linkedHashMap newVarDom
